...
class Checkout extends SetupBase {
  ...
  protected function setupCheckoutLocal(JobInterface $job, $details) {
    ...
    if (!is_dir($source_dir)) {
      Output::error("Directory error", "The source directory <info>$source_dir</info> does not exist.", $this->output);
      $job->error();
      return;
    }
    ...
    if (!($directory = $this->validateDirectory($job, $checkout_dir))) {
      ...
      Output::error("Directory error", "The checkout directory <info>$directory</info> is invalid.", $this->output);
      $job->error();
      return;
    }
    $this->output->writeln("<comment>Copying files from <options=bold>$source_dir</options=bold> to the local checkout directory <options=bold>$directory</options=bold> ... </comment>");
...
#    $this->exec("cp -r $source_dir/. $directory", $cmdoutput, $result);
    ...
    if ($result !== 0) {
      Output::error("Copy error", "Error encountered while attempting to copy code to the local checkout directory.", $this->output);
      $job->error();
      return;
    }
  ...
  protected function setupCheckoutGit(JobInterface $job, $details) {
    ...
    $this->output->writeLn("<comment>Performing git checkout of $repo $git_branch branch to $directory.</comment>");
    // TODO: Make sure target directory is empty
    $git_depth = '';
    ...
    $this->output->writeLn("Git Command: $cmd");
    $this->exec($cmd, $cmdoutput, $result);
    if ($result !==0) {
      // Git threw an error.
      Output::error("Checkout Error", "The git checkout returned an error.  Error Code: $result", $this->output);
      $job->error();
      return;
    }
    ...
    $this->exec($cmd, $cmdoutput, $result);
    $this->output->writeLn("<comment>Git commit info:</comment>");
    $this->output->writeLn("<comment>\t" . implode($cmdoutput));
