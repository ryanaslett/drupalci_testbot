...
class WebEnvironment extends PhpEnvironment {
  ...
  public function run(JobInterface $job, $data) {
    ...
    $this->output->writeLn("<info>Parsing required Web container image names ...</info>");
    $containers = $job->getExecContainers();
    $containers['web'] = $this->buildImageNames($data, $job);
    $valid = $this->validateImageNames($containers['web'], $job, $this->output);
    if (!empty($valid)) {
  ...
  protected function buildImageNames($data, JobInterface $job) {
    ...
    foreach ($data as $key => $php_version) {
      ...
      $this->output->writeLn("<comment>Adding image: <options=bold>drupalci/web-$php_version</options=bold></comment>");
    }
    return $images;
  }
