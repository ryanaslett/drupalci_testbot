...
class ConfigSetCommand extends DrupalCICommandBase {
  ...
  public function execute(InputInterface $input, OutputInterface $output) {
    ...
    foreach ($arguments as $argument) {
      ...
      if (count($parsed) != 2) {
        $this->logger->error("<error>Unable to parse argument.</error>");
        $this->logger->info("<comment>Please provide both a variable name and value formatted as <options=bold>variable_name=variable_value</options=bold></comment>");
        return;
      }
      ...
      if ($existing = getenv($key)) {
        ...
        $this->logger->error("<error>The <option=bold>$key</option=bold> setting has been set via an environment variable and can not be set via the console.</error>");
        $this->logger->info("<comment>To override this value, provide it on the command line as part of the drupalci invocation.");
        $this->logger->info("<comment>Example: </comment> $key=$value ./drupalci [command]");
      }
      ...
      elseif (in_array($key, array_keys($config))) {
        ...
        $this->logger->info("<info>The <option=bold>$key</option=bold> variable already exists.</info>");
        ...
        $this->logger->info("<info>Setting the value of the <option=bold>$key</option=bold> variable to <option=bold>$value</option=bold>");
        $helper->setConfigVariable($key, $value);
      }
    }
  }
